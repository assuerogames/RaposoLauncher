...
class ModDownloader(tk.Toplevel):
    ...
    def open_mod_details_view(self, project_id, title, author):
        ...
        # --- MUDANÇA AQUI ---
        # Prepara uma lista para guardar as referências das imagens da galeria
        self.gallery_photos = []
        # --- FIM DA MUDANÇA ---
    ...
    def close_mod_details_view(self):
        ...
        # --- MUDANÇA AQUI ---
        # 2. Limpa a lista de referências de fotos
    ...
    def _fetch_and_show_details(self, project_id, content_frame, loading_label, title, author):
        ...
        try:
            ...
            # --- MUDANÇA AQUI: Processa o Markdown para Texto Puro ---
            try:
                # 1. Converte o Markdown (com ###) para HTML (com <h3>)
                md = MarkdownIt()
                html_content = md.render(body_markdown)
                # 2. Remove as tags HTML (<h3>) e deixa só o texto
                plain_text_description = strip_tags(html_content)
            except Exception as e:
                print(f"Erro ao limpar Markdown: {e}")
                plain_text_description = body_markdown # Fallback
            # --- FIM DA MUDANÇA ---
            ...
            def _populate_ui():
                ...
                # --- MUDANÇA AQUI ---
                # 1. Mostra a DESCRIÇÃO COMPLETA e LIMPA
                ttk.Label(
                    content_frame, 
                ...
                # 2. Adiciona o Botão para abrir a página completa
                ...
                if not project_slug:
                    open_button.config(state="disabled", text="Não foi possível encontrar a página do mod")
                ...
                if gallery_data:
                    ...
                    for i, image_info in enumerate(gallery_data[:5]):
                        ...
                        img_label = ttk.Label(
                            gallery_frame,
                            text=f"Carregando imagem {i+1}...",
                            bootstyle="secondary"
                        )
                        img_label.pack(pady=5)
                        
                        threading.Thread(
                            target=self._load_gallery_image, 
                            args=(img_label, image_url, 550), 
                            daemon=True
                        ).start()
                # --- FIM DA GALERIA ---
